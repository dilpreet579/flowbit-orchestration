    ...
    if (engine === "n8n") {
      return {
        ...
        nodes: data.data?.resultData?.runData
          ...
          : [],
        messages: data.data?.resultData?.runData
          ? Object.entries(data.data.resultData.runData)
              .filter(
                ([nodeName, nodeData]: [string, any]) =>
                  ...
                  nodeName.toLowerCase().includes("telegram"),
              )
              .flatMap(([nodeName, nodeData]: [string, any]) => {
                if (!nodeData[0]?.data) return []

                // Extract message data based on node type patterns
                if (nodeName.toLowerCase().includes("email")) {
                  return [
                    {
                      id: `msg-${data.id}-${nodeName}`,
                      direction: "outgoing",
                      recipient: nodeData[0].data.to || nodeData[0].data.recipient,
                      content: nodeData[0].data.text || nodeData[0].data.html || nodeData[0].data.subject,
                      timestamp: new Date(data.startedAt)
                        .toLocaleDateString("de-DE", {
                          day: "2-digit",
                          month: "2-digit",
                          year: "numeric",
                ...
                return [
                  {
                    id: `msg-${data.id}-${nodeName}`,
                    direction: nodeData[0].data.direction || "outgoing",
                    recipient: nodeData[0].data.to || nodeData[0].data.recipient,
                    sender: nodeData[0].data.from || nodeData[0].data.sender,
                    content: nodeData[0].data.text || nodeData[0].data.content || nodeData[0].data.message,
                    timestamp: new Date(data.startedAt)
                      .toLocaleDateString("de-DE", {
                        day: "2-digit",
                        month: "2-digit",
        ...
        messages: data.outputs
          ? Object.entries(data.outputs)
              ...
              .flatMap(([nodeName, nodeData]: [string, any]) => {
                ...
                // Extract message data based on node patterns
                if (nodeName.toLowerCase().includes("chat")) {
                  return (Array.isArray(nodeData.data.messages) ? nodeData.data.messages : []).map((msg: any) => ({
                    id: `msg-${data.id}-${nodeName}-${Math.random().toString(36).substring(7)}`,
      ...
      <DialogContent className="max-w-6xl max-h-[90vh]">
        ...
        <Tabs defaultValue="overview" className="w-full">
          ...
          <TabsContent value="messages" className="space-y-4">
            <MessagesView
              messages={executionDetails.messages || []}
              maxHeight="96"
              showWorkflowInfo={false}
              emptyMessage="No message logs available for this execution"
            />
          </TabsContent>
