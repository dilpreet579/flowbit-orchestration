        ...
        messages: [
          {
            ...
            content: "This is a sample email message sent during the workflow execution.",
        ...
        messages: [
          {
            direction: "incoming",
            sender: "user",
            content: "Can you help me with my data processing workflow?",
          ...
          },
          {
            direction: "outgoing",
            content:
              "I'd be happy to help with your data processing workflow. What specific issues are you experiencing?",
            timestamp: data.timestamp,
            metadata: {
              role: "assistant",
            ...
            <>
              ...
              <Tabs defaultValue="overview" className="w-full">
                ...
                <TabsContent value="messages" className="space-y-4">
                  <ScrollArea className="h-96">
                    {executionDetails.messages && executionDetails.messages.length > 0 ? (
                      <div className="space-y-2">
                        {executionDetails.messages.map((message: any, index: number) => (
                          <Card
                            key={index}
                            className={
                                ...
                                <div className="flex-1">
                                  <div className="flex justify-between mb-1">
                                    <div className="text-xs font-medium">
                                      ...
                                      {message.recipient && (
                                        <span className="text-muted-foreground ml-1">to {message.recipient}</span>
                                      )}
                                      {message.sender && (
                                        <span className="text-muted-foreground ml-1">from {message.sender}</span>
                                      )}
                                    </div>
                                    <div className="text-xs text-gray-500">
                                      {message.timestamp || executionDetails.startTime}
                                    </div>
                                  </div>
                                  ...
                                  {message.metadata && (
                                    <div className="mt-2 text-xs text-muted-foreground">
                                      <Button
                                        ...
                                        onClick={() => {
                                          const metadataEl = document.getElementById(`sample-message-metadata-${index}`)
                                          if (metadataEl) {
                                            metadataEl.classList.toggle("hidden")
                                          }
                                        }}
                                      >
                                        Metadata <ChevronDown className="h-3 w-3 ml-1" />
                                      </Button>
                                      <pre
                                        id={`sample-message-metadata-${index}`}
                                        className="hidden mt-2 bg-muted p-2 rounded text-xs overflow-auto"
                                      >
                      ...
                      <div className="text-center py-8 text-gray-500">
                        <Info className="w-8 h-8 mx-auto mb-2" />
                        <p>No message logs available for this execution</p>
                      </div>
                    )}
                  </ScrollArea>
                </TabsContent>
